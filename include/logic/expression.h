...
namespace pram::inline logic
{
  ...
  protected:
    ...
    public:
      ...
      const static ExprOp RootExpr;

      template <typename Func, typename ...Args>
      constexpr decltype(auto) dispatch(const ExprNode& node, Func&& func, Args&& ...args) const {
          switch(exprKind){
            case ROOT:
              return std::forward<Func>(func)(reinterpret_cast<const RootExprNode&>(*this), std::forward<Args>(args)...);

            case TERMINAL:
              return std::forward<Func>(func)(reinterpret_cast<const TerminalExprNode&>(*this), std::forward<Args>(args)...);

            case UNARY:
              return std::forward<Func>(func)(reinterpret_cast<const UnaryExprNode&>(*this), std::forward<Args>(args)...);

            case BINARY:
              return std::forward<Func>(func)(reinterpret_cast<const BinaryExprNode&>(*this), std::forward<Args>(args)...);

            case TERNARY:
              return std::forward<Func>(func)(reinterpret_cast<const TernaryExprNode&>(*this), std::forward<Args>(args)...);

            case VARIADIC:
              return std::forward<Func>(func)(reinterpret_cast<const VariadicExprNode&>(*this), std::forward<Args>(args)...);
          }
      }
    };
  private:
  ...
  public:
    [[nodiscard]] constexpr virtual bool sameKindAs(const ExprNode& other) const{
  ...
  const ExprNode::ExprOp ExprNode::ExprOp::RootExpr{ROOT};

  constexpr struct RootExprNode : public ExprNode{constexpr RootExprNode() : ExprNode(ExprOp::RootExpr){}} RootNode{};
  ...
  protected:
    ...
    public:
      ...
      constexpr decltype(auto) dispatch(const TerminalExprNode& node, Func&& func, Args&& ...args) const {
        ...
        switch(terminalKind){
          case Op::VALUE:
            return std::forward<Func>(func)(reinterpret_cast<const ValueNode&>(node), std::forward<Args>(args)...);
          case Op::NIL:
            return std::forward<Func>(func)(reinterpret_cast<const NullNode&>(node), std::forward<Args>(args)...);
        }
      }
    };
